    ...
    <>
      {totalPages > 1 && (
        ...
        >
          {/* previous */}
          {hasPrevPage ? (
            <Link href={indexPageLink ? `${section ? "/" + section : "/"}` : `${section ? "/" + section : ""}?index=${currentPage - 1}`}
            ...
            <span className={`${!hasPrevPage && "opacity-0"} inline-flex h-11 w-11 items-center justify-center rounded px-2 py-2 text-primary hover:bg-primary hover:text-body`}>
              <TfiAngleLeft />
            </span>
          )}
          ...
          {pageList.map((pagination, i) => (
            <React.Fragment key={`page-${i}`}>
              {pagination === currentPage ? (
                ...
                <Link href={i === 0 ? `${section ? "/" + section : "/"}` : `${section ? "/" + section : ""}?index=${pagination}`}
                  passHref
                  aria-current="page"
          ...
          {hasNextPage ? (
            <Link
              href={`${section ? "/" + section : ""}?index=${currentPage + 1}`}
              className="inline-flex h-11 w-11 items-center justify-center rounded px-2 py-2 text-primary hover:bg-primary hover:text-body">
              <TfiAngleRight />
            </Link>
          ) : (
